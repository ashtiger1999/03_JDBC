...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/* 3. SQL 작성 */
			// ** 주의사항 **
			// JDBC 코드에서 SQL 작성 시 세미콜론(;)을 작성하면 안된다
			...
			// -> 자원 반환을 하지 않으면 DB와 연결된 Connection이 그대로 남아있어서
			//    다른 클라이언트(ex. java 프로그램)가 추가적으로 연결되지 못하는 문제가 발생할 수 있다.
			// -> DBMS는 최대 Connection 수 개수 제한을 하고 있기 때문에
			
			try {
				
				// 개체가 만들어진 순서의 역순으로 close 수행 권장
				if(rs!=null) rs.close();
